...
document.querySelector('#btn1').addEventListener('click',()=>{
    ...
    const regExp2 = new RegExp("java");
    // "java"와 일치하는 문자열이 있는지 검사하는 정규 표현식

    // 확인하기
    const str1 = "저희는 지금 javascript를 공부하고 있습니다.";
    const str2 = "jsp(java server page)도 곧 할 것입니다.";
...
document.querySelector('#btn2').addEventListener('click',()=>{
    ...
    div1.innerHTML += "/[abcd]/, subscribe : " + regExp1.test("subscribe") + "<hr>";
    div1.innerHTML += "/[abcd]/, qwerty : " + regExp1.test("qwerty") + "<hr>";

    // ^ (캐럿)
    const regExp2 = /^group/;
    div1.innerHTML += "/^group/, group study : " + regExp2.test("group study") + "<hr>";
    div1.innerHTML += "/^group/, my group : " + regExp2.test("my group") + "<hr>";
    div1.innerHTML += "/^group/, gro up : " + regExp2.test("gro up") + "<hr>";
    
    // $ (달러)
    const regExp3 = /group$/;
    div1.innerHTML += "/^group/, group study : " + regExp3.test("group study") + "<hr>";
    div1.innerHTML += "/^group/, my group : " + regExp3.test("my group") + "<hr>";
    div1.innerHTML += "/^group/, gro up : " + regExp3.test("gro up") + "<hr>";
})
...
document.getElementById('inputPno').addEventListener('keyup',(e)=>{
    ...
    if(e.target.value.length == 0) {
        span.innerText = '주문등록번호를 작성해주세요.';
        span.classList.remove('confirm','error');
        return;
    }

    // 생년월일(6)-고유번호(7)
    const regExp = /^\d{2}[1-12]{2}[1-31]{2}-[1-2]{1}\d{6}$/;
